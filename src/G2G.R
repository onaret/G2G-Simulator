...
to_study_design <- function(structure) {
	...
	do.call(rbind, lapply(1:ncol(structure), function(pop_num) {
		do.call(rbind, lapply(1:nrow(structure), function(strain_num) {
			if(structure[strain_num,pop_num] > 0) data.frame(Population = paste0("P",pop_num), Strain = rep(chartr("123456789", "ABCDEFGHI", strain_num), structure[strain_num,pop_num]) ) }))})) }
...
##@G
get_AA <- function(study_design, AA.scenarios, SNP.data, associated_SNPs = NULL, generate_AAs = generate_AAs_standard) {
...
generate_AAs_standard <- function(AA.freq, scenario, study_design, associated_SNPs) {
	strains = levels(study_design$Strain)
	populations = levels(study_design$Population)
...
generate_AAs_with_noise <-  function(AA.freq, scenario, study_design, associated_SNPs) {
	strains = levels(study_design$Strain)
	populations = levels(study_design$Population)
	#if(trace == TRUE) print(paste(Sys.time(),"Generating Viral output for", scenario$id_tag, sep=" : "))
	res = sapply(1:nrow(AA.freq), function(aa_num) {
