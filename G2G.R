...
test_G2G_setup <- function(study_design, scenario, fst_pop_strat=NA, fst_pop_bias=NA, fst_strain_strat=NA, fst_strain_bias=NA, tag = NULL, get_viral = generate_AAs) {
	scenario_name = paste(deparse(substitute(scenario)), if(!is.null(tag)) paste0("_", tag))
...
generate_biological_tag <- function(nb_ele, tag_size_range) {
	give_me <-function() round(runif(1, min = tag_size_range["min"], max = tag_size_range["max"]))
	increment_tag <- function(sequence=c()) {
		if(nb_ele - sum(sequence) < tag_size_range["max"]) c(sequence, nb_ele - sum(sequence))
		else increment_tag(c(sequence, give_me()))}
	increment_tag()}
...
parse_G2G_config <- function(study_design, ...) {
	...
	#	calls = lapply(calls, function(call) eval(call$study_design = quote(study_design)))
	set_env("AA", 0)
	set_env("SNP", 0)
...
get_AA <- function(study_design, AA.scenarios, SNP.data) {
...
analyse_G2G <- function(SNP, AA, study_design, nb_cpu, WO_correction = F, W_human_groups = F, W_strain_group = F, W_both_groups = F, W_human_PC = F, W_strain_PC = F, W_both_PC = F, W_non_linear_PC =F) { #SNP
	...
	analyse_AA <- function(Y) {
		Filter(length, list(
			...
			`W_non_linear_PC` = if(W_non_linear_PC == T) unlist(apply(SNP, 2, function(X) coef(summary(glm(Y~X+AA_NL_PC)))[,4][2]))))}
